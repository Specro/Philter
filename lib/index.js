  ...
  _.forEach(elements, (element) => {
    ...
    _.forEach($(element).data(), (value, key) => {
      ...
      switch (key) {
        ...
        case 'duotone':
          filterCount.duotone++
          rule.default += filters.duotone(filterCount.duotone)
          getDuotoneFilter(values[0], values[1], filterCount.duotone, promises, (filter) => {
            svg += filter
          })
          if (values[2] && values[3]) {
            filterCount.duotone++
            rule.hover += filters.duotone(filterCount.duotone)
            getDuotoneFilter(values[2], values[3], filterCount.duotone, promises, (filter) => {
              svg += filter
            })
          }
          break
...
function getColorFilter(color, opacity, id, promises, cb) {
  let filter = ''
  promises.push(
    fs.readFileAsync(`${__dirname}/svg/color.svg`, 'utf-8').then((data) => {
...
function getAnaglyphFilter(offset, id, promises, cb) {
  let filter = ''
  promises.push(
    fs.readFileAsync(`${__dirname}/svg/anaglyph.svg`, 'utf-8').then((data) => {
      let $ = cheerio.load(data, {recognizeSelfClosing: true});
...
/**
* Parse duotone filter and pass it to the callback
* @param {String} color1 First filter color
* @param {String} color2 Second filter color
* @param {Integer} id Filter ID
* @param {Array} promises List of readFileAsync promises
* @param {Function} cb Callback
*/
function getDuotoneFilter(color1, color2, id, promises, cb) {
  let filter = ''
  color1 = rgb(color1);
  color2 = rgb(color2);
  promises.push(
    fs.readFileAsync(`${__dirname}/svg/duotone.svg`, 'utf-8').then((data) => {
      let $ = cheerio.load(data, {recognizeSelfClosing: true});
      $('filter').attr('id', 'duotone-' + id)
      $('fefuncr').attr('tablevalues', color1.red/255+' '+color2.red/255)
      $('fefuncg').attr('tablevalues', color1.green/255+' '+color2.green/255)
      $('fefuncb').attr('tablevalues', color1.blue/255+' '+color2.blue/255)
      filter = $.html()
      cb(filter)
    }).catch((err) => {
      throw err
...
function getVintageFilter(id, promises, cb) {
  let filter = ''
  promises.push(
    fs.readFileAsync(`${__dirname}/svg/vintage-${id}.svg`, 'utf-8').then((data) => {
      cb(data)
    }).catch((err) => {
      throw err
    })
